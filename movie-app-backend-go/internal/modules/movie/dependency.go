...
type TMDBModule struct {
	TMDBController *controllers.TMDBController
}

func NewTMDBModule(db *gorm.DB) *TMDBModule {
	tmdbService := services.NewTMDBService(db)
	return &TMDBModule{
		TMDBController: controllers.NewTMDBController(tmdbService),
	}
}

func RegisterTMDBRoutes(rg *gin.RouterGroup, module *TMDBModule, mf *middleware.Factory) {
	rg.POST("/admin/movies/fetch-tmdb", mf.Auth(), mf.RequirePermission("movies.create"), module.TMDBController.ManualFetch)
}
