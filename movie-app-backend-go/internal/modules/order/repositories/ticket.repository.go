...
func (r *TicketRepository) GetBookedSeats(scheduleID uint) ([]uint, error) {
	var seatNumbers []uint
	err := r.DB.Model(&models.Ticket{}).
		Where("schedule_id = ? AND status != ?", scheduleID, enums.TicketStatusCancelled).
		Pluck("seat_number", &seatNumbers).Error
	return seatNumbers, err
}
