...
type GroupedScheduleDetails struct {
	ID        uint       `json:"id"`
	StudioID  uint       `json:"studio_id"`
	StartTime time.Time  `json:"start_time"`
	EndTime   time.Time  `json:"end_time"`
...
type ScheduleGroupedByMovieResponse struct {
	Movie     MovieInfo                `json:"movie"`
	Schedules []GroupedScheduleDetails `json:"schedules"`
}

type PaginatedGroupedScheduleResponse struct {
	Page      int                              `json:"page"`
	PerPage   int                              `json:"per_page"`
	Total     int64                            `json:"total"`
	TotalPage int                              `json:"total_page"`
	Data      []ScheduleGroupedByMovieResponse `json:"data"`
}